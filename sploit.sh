...
echo -e "$red                       ▶Coded by$grn BetaSploit$red◀$rset"
echo -e "$red                      ⫸$ylo  DARK WORLD OF LAGIT  ⫷$rset"
echo 
# Functions
...
then
am start -a android.intent.action.VIEW -d https://youtube.com/channel/UCuLxdEoRfo7pKjn6xMJd69w > /dev/null 2>&1
banner
menu
elif [ $optnz = "6" ];
then
am start -a android.intent.action.VIEW -d https://codershome.code.blog > /dev/null 2>&1
banner
menu
elif [ $optnz = "7" ];
then
am start -a android.intent.action.VIEW -d https://chat.whatsapp.com/KArsTgYYDS4JnlSGGjbT0g > /dev/null 2>&1
banner
menu
elif [ $optnz = "8" ];
then
...
echo -e '\e[91m
         ...
         ────▌──▐────
'
echo -e "\e[93m                  CALL ME \e[92mBetaSploit" | pv -qL 10
echo -e "\e[93m             IAM FROM \e[92mYoutube" | pv -qL 10
...
echo -e "\e[93m        MY WEBSITE:\e[92m codershe.code.blog\e[0m" | pv -qL 10
...
while git clone https://github.com/BuggHub/sploit 2> /dev/null; do 
    printf  "\e[92m▓▓▓▓▓▓▓▓▓▓▓▓▓\e[0m"
    sleep 1
done
printf "\e[96m]\e[0m"
...
if [ "$p" = "y" ];
then
am start -a android.intent.action.VIEW -d https://codershome.code.blog 2>/dev/null
clear
baner1
menu
elif [ "$p" = "t" ];
